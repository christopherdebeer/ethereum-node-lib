  ...
  async.auto({
    getSender: function (cb2) {
      ...
      self.trie.get(address, function (err, account) {
        ...
        var balance = bignum.fromBuffer(fromAccount.balance);
        //checks senders balance
        if (balance.ge(bignum(tx.getUpfrontCost()))) {
          fromAccount.nonce = bignum.fromBuffer(fromAccount.nonce).add(1).toBuffer();
          cb2(err, fromAccount);
        } else {
          cb2('not enought gas in sender\'s account');
        }
      });
    },
    ...
    processAccounts: ['getSender', 'getReceiver',
      function (cb2) {
        ...
        var fromBalance = bignum.
        fromBuffer(fromAccount.balance).
        sub(bignum.fromBuffer(tx.value));

        //check if senders account has enough
        if (fromBalance.ge(0)) {
          fromAccount.balance = fromBalance.toBuffer();
        } else {
          cb('to enought in sender\'s account');
          return;
        }

        if (toAccount.isContract() && tx.data.toString('hex') !== '00') {
          var vm = new VM();
          vm.block = block;
          vm.saveAccount(function (account, done) {
            //self.trie.put();
            done();
          });
          vm.loadAccount(function (account, done) {
            //self.tri.get();
            done();
          });
          vm.run(tx, function(returnValue, account){
            cb2();
          });
        } else {
          cb2();
