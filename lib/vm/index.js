...
internals.VM.prototype.run = function (tx, saveAccount, cb) {
    ...
    function runCode() {
        ...
        var gasUsed = tx.gasLimit - gasLeft;
        self.saveAccount({storage:storage}, function(){
            if (cb) cb(new Buffer(returnValue), gasUsed, {storage:storage});
        });
    }
    ...
    gasLeft -= tx.dataFee();
    if (tx.type === "contract") {
        gasLeft -= fees.getFee("TRANSACTION");
        code = tx.init;
        runCode();
    } else {
