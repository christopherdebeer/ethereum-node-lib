    ...
    socket.on('data', function (data) {
        ...
        while (data.length) {
            ...
            if (parsedData) {
                ...
                parsedData[0] = 'message.' + messageType;
                ...
                parsedData.pop();
                parsedData[0] = messageType;
                parsedData.unshift('message');
                self.emit.apply(self, parsedData);
            }
        }
    });
...
internals.Peer.prototype.sendGetChain = function (parents, count, cb) {
    ...
    parents = parents.map(function (d) {
        return !Buffer.isBuffer(d) ? new Buffer(d, 'hex') : d;
    });

    message = message.concat(parents);
    message.push(count);
    this.sendMessage(message, cb);
};
...
internals.parseTxs = function (payload) {
    ...
    for (var i = 1; i < payload.length; i++) {
        ...
        console.log(etherUtils.bufferToJSON(payload[i]));
